...
# build TF-IDF index for content similarity (title + description)
@st.cache_resource
def build_text_index(df):
    corpus = (df["title"].fillna("") + " " + df["description"].fillna("")).astype(str).tolist()
    vect = TfidfVectorizer(max_features=7000, ngram_range=(1,2))
    if len(corpus) == 0:
        return vect, None
    X = vect.fit_transform(corpus)
    return vect, X

tfidf_vect, tfidf_X = build_text_index(prod_df)

# mappings
itemid_to_idx = {str(iid): idx for idx, iid in enumerate(prod_df["item_id"].astype(str).tolist())}
idx_to_itemid = {v: k for k, v in itemid_to_idx.items()}
...
def content_similar(item_id, top_k=20):
    if tfidf_X is None or item_id not in itemid_to_idx:
        return []
    idx = itemid_to_idx[item_id]
    vec = tfidf_X[idx]
    sims = cosine_similarity(tfidf_X, vec).flatten()
    top = sims.argsort()[::-1]
    results = []
    for i in top:
        if i == idx:
        ...
        if len(results) >= top_k:
            break
    return [idx_to_itemid[i] for i,_ in results]
...
def product_card(row):
    ...
    st.markdown(f"Price: ₹{int(row.get('price',0))}")
    ...
    if current_item:
        ...
        with right_col:
            ...
            for s in sim:
                ...
                st.write(f"- {r['title']} — ₹{int(r['price'])}")
